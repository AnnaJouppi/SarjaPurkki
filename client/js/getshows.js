...
async function renderShows() {
  ...
  console.log(shows);
  if (shows.length === 0) {
    let noresults = `<h2 class="no-results-text" >Sorry, your search didn't match any shows 😐</h2>`;
    ...
    shows.forEach((item) => {
      ...
      if (item.show.image === null) {
        ...
        let makeitstring = JSON.stringify(
          item.show.image ? item.show.image.medium : item.show.image.original
        );
      ...
      let url = `<p><a href="${item.show.url}">${item.show.url}</a></p>`;
      ...
      if (item.show.summary === null) {
        let nosummary =
          '<p class="showsummary"><em>No summary available.</em></p>';
        div += nosummary;
      } // jos löytyy:
      else {
      ...
      if (item.show.premiered === null && item.show.ended === null) {
        let noneavailabe = `<p class="showotherinfo"><span class="bold-text">No production dates available.</span></p>`;
        div += noneavailabe;
      }
      // Jos lähetyksen alkua ei löydy
      else if (item.show.premiered === null) {
        let nopremiere = `<p class="showotherinfo"><span class="bold-text">Active:</span> Not known - ${item.show.ended}</p>`;
        div += nopremiere;
      }

      // Jos lähetyksen endiä ei löydy
      else if (item.show.ended === null) {
        let noending = `<p class="showotherinfo"><span class="bold-text">Active:</span> ${item.show.premiered} - now</p>`;
        div += noending;
      }
      // jos molemmat löytyy
      else {
        let showairing = `<p class="showotherinfo"><span class="bold-text">Active:</span> ${item.show.premiered} - ${item.show.ended}</p>`;
        div += showairing;
      }

      // Genret ja URL
      // Jos genrejä ei löydy

      if (item.show.genres && item.show.genres.length > 0) {
        let genres = `<p class="showgenres">${item.show.genres}</p>`;
        div += genres;
      }
      div += url;
